...
import com.print.model.ReversalModel;
...
public class TransferPacketThread extends Thread{
	...
	private void pack(HashMap<String, String> dataMap){
		try{
			ArrayList<FieldModel> list = TransferLogic.getInstance().parseConfigXML("con_req_"+ this.transferCode +".xml");
			...
			StringBuilder mabsb = new StringBuilder();
			...
			String mabStr = mabsb.toString();
			String macStr = macsb.toString(); 
			
			if (!"".equals(mabStr)){ // 需要进行MAC计算
				sendJSONStringer.key("fieldMAB").value(mabStr.substring(0, mabStr.length()-1));
				
				CalcMacHandler calcHandler = new CalcMacHandler();
				
				FSKOperator.execute("Get_MAC|int:0,int:1,string:null,string:"+macStr, calcHandler);
		...
		try{
			...
			if (this.transferCode.equals("500000001")){
				...
				if (null != mabField && !mabField.trim().equals("")){
					...
					FSKOperator.execute("Get_CheckMAC|int:0,int:0,string:null,string:"+macsb.toString()+receiveFieldMap.get("fieldMAC"), checkHandler);//  计算MAC的数据+MAC（8字节）
		...
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				if (cmdReturn.Return_Result == 0){ // mac计算成功
					try {
						sendJSONStringer.key("fieldMAC").value(Util.BytesToString(cmdReturn.Return_PSAMMAC));
						sendJSONStringer.endObject();
						Log.e("send JSON", sendJSONStringer.toString());
