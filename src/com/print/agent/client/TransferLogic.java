...
import com.print.activity.RecordDeviceActivity;
import com.print.activity.SettlementSuccessActivity;
import com.print.activity.TimeoutService;
import com.print.agent.client.db.AnnouncementDBHelper;
import com.print.agent.client.db.ReversalDBHelper;
import com.print.agent.client.db.TransferSuccessDBHelper;
import com.print.agent.client.db.UploadSignImageDBHelper;
import com.print.fsk.FSKOperator;
import com.print.model.AnnouncementModel;
import com.print.model.FieldModel;
import com.print.model.TransferSuccessModel;
...
public class TransferLogic {
	...
	private void actionDone(HashMap<String, String> fieldMap){
		...
		} else if("200310001".equals(transferCode)){ // 银行卡余额查询
			this.queryBalanceDone(fieldMap);
			
		} else if("200000022".equals(transferCode)){ // 收款
			this.receiveTransDone(fieldMap);
			
		} else if ("200200023".equals(transferCode)){ // 收款撤销
	...
	private void loginDone(HashMap<String, String> fieldMap) {
		...
		try {
			...
			if (null != fieldMap) {
				...
				// BaseActivity.getTopActivity().startService(new Intent("com.print.timeoutService"));
				
				// 登陆成功，跳转到菜单界面
				BaseActivity.getTopActivity().showDialog(BaseActivity.PROGRESS_DIALOG, "登录成功");
				BaseActivity.getTopActivity().startActivity(new Intent("com.print.lrcatalog"));
	...
	private void signDone(final HashMap<String, String> fieldMap){
		...
		try{
			if (null != newKey && !"".equals(newKey)){
				int macLength = Integer.parseInt(newKey.substring(0, 3)); 
				macKey = newKey.substring(3, 3+macLength);
				
				int pinLength = Integer.parseInt(newKey.substring(3+macLength, 3+3+macLength));
				pinKey = newKey.substring(3+3+macLength, 3+3+macLength+pinLength);
				
				int stackLength = Integer.parseInt(newKey.substring(3+3+macLength+pinLength, 3+3+3+macLength+pinLength));
				stackKey = newKey.substring(3+3+3+macLength+pinLength, 3+3+3+macLength+pinLength+stackLength);
		...
		sb.append("Get_RenewKey|string:").append(macKey).append(",string:").append(pinKey).append(",string:").append(stackKey);
	...
	private void queryHistoryGroupDone(HashMap<String, String> fieldMap){
		...
		if (null != sendMap && sendMap.containsKey("BeginDate") && sendMap.containsKey("EndDate")){
			Intent intent = new Intent("com.print.queryTransferHistoryGroupActivity");
	...
	private void queryHistoryListDone(HashMap<String, String> fieldMap){
		...
		if (null != sendMap && sendMap.containsKey("totalCount")){
			Intent intent = new Intent("com.print.queryTransferHistoryList");
	...
	private void receiveTransDone(HashMap<String, String> fieldMap){
		...
		try{
			// 旧 001903   新 001917
			if (AppDataCenter.getValue("__TERSERIALNO").startsWith("001917")) { // 001903
				// 打印
				Intent intent = new Intent("com.print.printReceipt");
	 ...
	 */
	 ...
	 public void uploadReceiptAction(HashMap<String, String> fieldsMap){
		// 静态演示模式下不上传签购单。
		 ...
		 try{
			...
			try{
				BaseActivity.getTopActivity().startService(new Intent("com.print.uploadSignImageService"));
	 ...
	 private void downloadAnnouncements(HashMap<String, String> fieldMap) {
		 try{
			 ...
			 BaseActivity.getTopActivity().startActivity(new Intent("com.print.announcementListActivity"));
	 ...
	 private void checkUpdateAPK(HashMap<String, String> fieldMap) {
		 Intent intent = new Intent("com.print.updateAPKService");
	 ...
	 public ArrayList<FieldModel> parseConfigXML(String confName) throws FileNotFoundException{
		ArrayList<FieldModel> list = new ArrayList<FieldModel>();
